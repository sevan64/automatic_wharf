...
void loop() {
    ...
    else
    {
        ...
        {
            HUSKYLENSResult = huskylens.read();
            printResult(result);
        }
    }
}
...
void printResult(HuskylensResult result){
    if (result.command == COMMAND_RETURN_BLOCK){
        ...
        Serial.println(String()+F("Arrow:xOrigin=")+result.xOrigin+F(",yOrigin=")+result.yOrigin+F(",xTarget=")+reult.xTarget+F(",yTarget=")+result.yTarget+F(",ID=")+result.ID);
        delay(500);
    }
    else{
